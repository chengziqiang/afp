...
def graph_dict(smiles_list, label_list=None, filename=None):

    try:
        graph_dict = pickle.load(open(filename+'.pkl',"rb"))
        print('graph dicts loaded from '+ filename+'.pkl')
        
    except:
        graph_dict = {}
        if label_list is not None:
            for i, smiles in enumerate(smiles_list):        
                graph_dict[smiles] = generate_graph(smiles, label=label_list[i])

            pickle.dump(graph_dict,open(filename+'.pkl',"wb"))
            print('graph dicts saved as '+ filename+'.pkl')
            
        else:
            for i, smiles in enumerate(smiles_list):        
                graph_dict[smiles] = generate_graph(smiles)            

    return graph_dict

if __name__ == '__main__':

    smiles_list = ['[H]N1C([H])([H])[C@]1([H])C#CC([H])([H])[H]','[H]/N=C(/OC([H])=O)C([H])([H])[H]','C1=CC=CC=C1', 'CNC', 'N', 'C', ]
#     label_list = [2.,3.,4.,4.34,5]
#     graph_dict = graph_dict(smiles_list, label_list, 'test')
    graph_dict = graph_dict(smiles_list, filename='test')
    print(np.concatenate(graph_dict['N'].atom[0],-1).shape)
    print(graph_dict['N'].bond[0].shape)
